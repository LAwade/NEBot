...
/*
 *                         ts3admin.class.php to TeamspeakAdmin
 *                         ------------------                    
...
/**
 * @file
 * The TeamspeakAdmin is a powerful api for communication with Teamspeak 3 Servers from your website! Your creativity knows no bounds!
 * 
 * @author      Stefan Zehnpfennig
 * @copyright   Copyright (c) 2017, Stefan Zehnpfennig
 * @version     1.0.3
 * @package     TeamspeakAdmin
 * 
 */
...
// *****************************************************************************
/**
 *  - New connection to TeaSpeak added
 *  - New methods for music bot administration
 *  - Class name change for better traceability
 * 
 *  @modify      Lucas Awade 
 *  @credits     Lendários Team - 2019 © Copyright - lendariosteam.com.br
 */

/**
 * \class TeamspeakAdmin
 * \brief The TeamspeakAdmin
 */
class TS3Admin {
//*******************************************************************************************	
//****************************************** Vars *******************************************
...
//******************************************************************************************
     ...
     * @return     array clientList 
     */
    function clientList($params = '-uid -away -voice -times -groups -info -country -icon -ip -badges') {
        if (!$this->runtime['selected']) {
            return $this->checkSelected();
        }
...
//*******************************************************************************************
    ...
    function connect() {
        ...
        if (!$socket) {
            ...
            $tmpVar = fgets($socket);
            if ((strpos($tmpVar, 'TS3') !== false) || (strpos($tmpVar, 'TeaSpeak') !== false)) {
                $tmpVar = fgets($socket);
     ...
     * @return	array	data
     */
    public function readChatMessage($type = 'textchannel', $keepalive = false, $cid = -1, $tracert = null) {
    ...
    public function readEventServer($keepalive = false, $cid = -1, $tracert = null) {
        $availTypes = array('textserver', 'textchannel', 'textprivate');
        $rtnData = array('success' => 0, 'data' => array('invokerid' => '', 'invokeruid' => '', 'invokername' => '', 'msg' => '', 'targetmode' => '', 'client_nickname' => '', 'client_unique_identifier' => '', 'client_totalconnections' => '', 'client_database_id' => '', 'clid' => ''));

        if (!$this->isConnected()) {
            $this->addDebugLog('script isn\'t connected to server');
            return $rtnData;
        }
        ...
        $this->executeCommand("servernotifyregister event=server", null);
        $this->executeCommand("servernotifyregister event=textprivate" . ($cid != -1 ? " id=$cid" : ""), null);
        $this->executeCommand("servernotifyregister event=textserver" . ($cid != -1 ? " id=$cid" : ""), null);

        $data = fgets($this->runtime['socket'], 4096);
        if (!empty($data)) {
            $msgData = explode(" ", $data);

            if(trim($msgData[0]) == "notifycliententerview" || trim($msgData[0]) == "notifytextmessage"){
                $rtnData['success'] = 1;
                foreach ($msgData as $param) {
                    $paramData = explode("=", $param);
                    $rtnData['data']['command'] = $msgData[0];
                    if (array_key_exists($paramData[0], $rtnData['data'])) {
                        $rtnData['data'][$paramData[0]] = $this->unescapeText(implode("=", array_slice($paramData, 1, count($paramData) - 1)));
                    }
                }
        ...
        if (!$keepalive){
            $this->serverNotifyUnregister('server');
        }
        return $rtnData;
    }
     ...
     * @author		toxiicdev (@toxiicdev.net)
     */
    public function serverNotifyUnregister($event = null) {

        $this->executeCommand("servernotifyunregister" . ($event ? " event=$event" : ""), null);
    }
